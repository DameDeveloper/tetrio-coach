...
Accuracy 0.962, **Macro F1-score 0.960**, 5-fold CV Macro F1 0.956 (±0.003). Labels are derived via K-Means clustering (k=8) of the 10-feature vectors to avoid circular reasoning, with noise-injection oversampling to balance classes.
...
A full academic paper outline accompanies this system, available in both languages:

- **English**: [TetrioCoach_Paper_Outline_EN.pdf](TetrioCoach_Paper_Outline_EN.pdf)
- **한국어 (Korean)**: [TetrioCoach_Paper_Outline.pdf](TetrioCoach_Paper_Outline.pdf)

Both versions are generated by `generate_paper_pdf_en.py` and `generate_paper_pdf.py` respectively, and cover the system architecture, methodology (K-Means + Gradient Boosting hybrid pipeline), results, case studies, and limitations.
